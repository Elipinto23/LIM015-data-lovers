...
describe("filtrarData", () => {
  // Resultados
  const tipoPokemon = data.pokemon.filter((pokemon) =>
...
describe("searchPokemon", () => {
  // Resultados
  const buscarNombre = data.pokemon.filter((pokemon) =>
    pokemon.name.includes("pikachu")
  );

  const buscarNumero = data.pokemon.filter(
    (pokemon) => parseInt(pokemon.num) == parseInt("025")
  );
...
describe("calculoEstadistico", () => {
  ...
  it("los top 10 segun CP", () => {
    const top10CP = calculoEstadistico(
      data.pokemon
        .sort(
          (a, b) => parseInt(b.stats["max-cp"]) - parseInt(a.stats["max-cp"])
        )
        .slice(0, 10)
    );
...
describe("calculoEstadPeso", () => {
  ...
  it("los top 10 segun Peso", () => {
    const top10Peso = calculoEstadPeso(
      data.pokemon
        .sort((a, b) => parseInt(b.size.weight) - parseInt(a.size.weight))
        .slice(0, 10)
    );
...
describe("calcularEstadVida", () => {
  ...
  it("top 10 segun nivel de vida", () => {
    const top10Vida = calcularEstadVida(
      data.pokemon
        .sort(
          (a, b) => parseInt(b.stats["max-hp"]) - parseInt(a.stats["max-hp"])
        )
        .slice(0, 10)
    );
